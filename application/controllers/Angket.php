...
class Angket extends CI_Controller {
...

	{
		...
		{	
		redirect(base_url('angket/nim/'.$something));
		}
...
public function nim()

	{
		...
		$this->data['bpm_angket_mahasiswas'] = $this->db->query("select *, (select ba.angket from bpm_angket as ba where ba.id = bam.id_angket) as angket from 20192_bpm_angket_mahasiswa as bam where bam.nim = ".$filter)->result();
